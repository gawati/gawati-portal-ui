...
import {
  convertEncodedStringToObject,
  setInRoute,
  convertObjectToEncodedString,
  editInRoute
...
class SearchContentColumnFilter extends BaseSearchContentColumn {
  ...
  convertRoutePropToXQuery = (paramQ) => {
    return xQueryFilterBuilder(convertEncodedStringToObject(paramQ)).join("");
  }
  ...
  getSearch(paramsObj) {
    ...
    let apiRecent = apiGetCall("filter", paramsObj);
        ...
        if (this._mounted) {
          if (parseInt(items.records, 10) === 0) {
            ...
            this.setState({
              ...
              to:
                parseInt(items.itemsfrom, 10) +
                parseInt(items.pagesize, 10) -
                1,
              records: parseInt(items.records, 10),
      ...
      tooltip: {
        ...
        formatter: function(params) {
          return (
            params.value[2] +
            " results containing " +
            xElements_scatter[params.value[0]]
          );
        }
      },
      xAxis: {
  ...
  getTimeline(paramsObj) {
    ...
    let apiTimeline = apiGetCall("timeline", paramsObj);
    // cancel the previous request
  ...
  generatePagination = () => {
    ...
    Object.keys(pagination).map(
      k =>
        (pagination[k] = !isInt(pagination[k])
          ? pagination[k]
          : parseInt(pagination[k], 10))
    );
  ...
  filterLink = (match, pageLang, query) => {
    if (match === undefined || match.params === undefined || !match.params.q) {
      return setInRoute("filter", {
        ...
        count: 10,
        lang: pageLang,
        q: convertObjectToEncodedString(query)
      });
    } else {
      return editInRoute(
        {
  ...
  onChartClick = (chartParams) => {
    ...
    let query =
      this.props.match === undefined || this.props.match.params.q === undefined
        ? {}
        : convertEncodedStringToObject(this.props.match.params.q);

    let yearLink = this.filterLink(
      this.props.match,
      pageLang,
      this.yearFilter(chartParams.name, query)
    );
  ...
  onChartClickBar = (chartParams) => {
    ...
    let countryLink = this.filterLink(
      this.props.match,
      pageLang,
      this.countryFilter(chartParams.name, query)
    );
  ...
  onChartClickPie = (chartParams) => {
    ...
    let languageLink = this.filterLink(
      this.props.match,
      pageLang,
      this.languageFilter(chartParams.name, query)
    );
  ...
  onChartClickScatter = (chartParams) => {
    ...
    let keywordLink = this.filterLink(
      this.props.match,
      pageLang,
      this.keywordFilter(chartParams.name, query)
    );
  ...
  render() {
    ...
    let content = (
      <div className={`main-col col-xs-12 col-lg-9 col-md-9 col-sm-12`}>
        <Tabs>
          <TabList>
            <Tab> {T("Document Results")} </Tab> <Tab> {T("Timeline")} </Tab>
          </TabList>
          <TabPanel>
